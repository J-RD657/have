...
local Theme = {
    ...
    Accent1 = Color3.fromRGB(130, 50, 240),
    Accent2 = Color3.fromRGB(80, 120, 255),
    Accent3 = Color3.fromRGB(180, 60, 255),
...
AddFunction("Movement", {
    ...
    Callback = function(state)
        ...
        if state then
            local char = LocalPlayer.Character
            if char then setCollision(char, false) end
            ...
            noclipConnection = RunService.Stepped:Connect(function()
                ...
                local currentChar = LocalPlayer.Character
                if currentChar then setCollision(currentChar, false) end
            end)
    ...
    Callback = function(state)
        if state then
            ...
            ammoConnection = RunService.Heartbeat:Connect(function()
                if not ToggleStates["InfiniteAmmo"] then
                ...
                for _, v in ipairs(tool:GetDescendants()) do
                    if v.Name == "Ammo" or v.Name == "Bullets" or v.Name == "Magazine" then
            ...
            ActiveConnections["InfiniteAmmo"] = ammoConnection
        else
    ...
    Callback = function(state)
        ...
        if state then
            ...
            local playerRemovingConn = Players.PlayerRemoving:Connect(function(player)
                if player.Character then unhighlightCharacter(player.Character) end
    ...
    Callback = function(state)
        if state then
            ...
            allFuelConnection = RunService.Heartbeat:Connect(function()
                ...
                for _, obj in ipairs(workspace:GetDescendants()) do
                    if obj:IsA("Model") and (obj:FindFirstChild("VehicleSeat") or obj:FindFirstChild("Seat")) then
                        for _, v in ipairs(obj:GetDescendants()) do
                            if v.Name == "Fuel" or v.Name == "Gas" or v.Name == "Petrol" then
                                if v:IsA("NumberValue") or v:IsA("IntValue") then
                                    v.Value = 100
                                end
                            end
                ...
                local function findAndClick(parent)
                    for _, child in ipairs(parent:GetDescendants()) do
                        if child:IsA("TextButton") or child:IsA("ImageButton") then
                            ...
                            local text = ""
                            if child:IsA("TextButton") then text = child.Text:lower() end
...
AddFunction("Utility", {
    Name = "AutoTaxiFarm",
    DisplayName = "出租车自动刷钱",
    Description = "自动接客并送至目的地",
    Type = "Toggle",
    DefaultState = false,
    Order = 7,
    Callback = function(state)
        if state then
            local taxiConnection
            taxiConnection = RunService.Heartbeat:Connect(function()
                if not ToggleStates["AutoTaxiFarm"] then
                ...
                -- 1. 寻找乘客（根据实际游戏修改对象名）
                local passenger = workspace:FindFirstChild("Passenger")
                if not passenger then
                    -- 没有乘客时尝试触发接单按钮
                    local startButton = LocalPlayer.PlayerGui:FindFirstChild("StartJobButton", true)
                ...
                -- 2. 开车到乘客位置
                local passengerRoot = passenger:FindFirstChild("HumanoidRootPart") or passenger:FindFirstChild("Torso")
                if passengerRoot then
                    local distance = (passengerRoot.Position - hrp.Position).Magnitude
                    if distance > 10 then
                        local moveDir = (passengerRoot.Position - hrp.Position).Unit
                        hrp.CFrame = hrp.CFrame + moveDir * 1.5
                    else
                        -- 3. 乘客上车后开往目的地
                        local destination = workspace:FindFirstChild("Destination")
                        if destination then
                            local destDist = (destination.Position - hrp.Position).Magnitude
                            if destDist > 10 then
                                local moveDir = (destination.Position - hrp.Position).Unit
                                hrp.CFrame = hrp.CFrame + moveDir * 1.5
                            else
                                -- 到达目的地，触发完成按钮
                                local finishButton = LocalPlayer.PlayerGui:FindFirstChild("FinishJobButton", true)
                                if finishButton and finishButton:IsA("TextButton") then
            ...
            ActiveConnections["AutoTaxiFarm"] = taxiConnection
        else
            if ActiveConnections["AutoTaxiFarm"] then
                ActiveConnections["AutoTaxiFarm"]:Disconnect()
                ActiveConnections["AutoTaxiFarm"] = nil
            end
        end
    end
})
...
if CurrentTab then RefreshContent(CurrentTab) end
